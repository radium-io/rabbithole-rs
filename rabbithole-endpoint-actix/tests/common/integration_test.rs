...
fn basic_crud_test() {
    ...
    let resp: ServiceResponse = send_request!(app, get, "{}/dogs/{}", path, first_dog_id);
    assert!(resp.status().is_success());
    let doc = resp_to_doc(resp);
    let (resource, _) = doc.into_single().unwrap();
    ...
    let resp: ServiceResponse = send_request!(app, get, "{}/people/{}", path, first_human_id);
    assert!(resp.status().is_success());
    let doc = resp_to_doc(resp);
...
fn relationship_test() {
    ...
    for master in &master_resources {
        let resp: ServiceResponse = send_request!(app, post, master, "{}/people", path);
        assert!(resp.status().is_success());
    }
    ...
    let resp: ServiceResponse = send_request!(
        app,
        delete,
    ...
    assert_eq!(resp.status(), StatusCode::NO_CONTENT);
    let resp: ServiceResponse = send_request!(app, get, "{}/people/{}", path, second_master_id);
    let doc = resp_to_doc(resp);
    ...
    assert_eq!(resp.status(), StatusCode::NO_CONTENT);
    let resp: ServiceResponse = send_request!(app, get, "{}/people/{}", path, first_master_id);
    let doc = resp_to_doc(resp);
    let (first_human_resource, included) = doc.into_single().unwrap();

...
impl TryFrom<syn::Field> for Field {
    ...
    fn try_from(item: syn::Field) -> Result<Self, Self::Error> {
        let ty = item.ty.clone();
        if let Some(syn::Meta::List(syn::MetaList { ref nested, .. })) =
            get_meta(&item.attrs)?.last()
        {
                            ...
                            (path.get_ident(), nested.last())
                        {
                            if let Some(inner_type) = nested_last.path().get_ident() {
                                if field_ty == "to_many" {
                                    return Ok(Field {
                                        item,
                                        ty: FieldType::ToMany(inner_type.clone()),
                                    });
                                } else if field_ty == "to_one" {
                                    return Ok(Field {
                                        item,
                                        ty: FieldType::ToOne(inner_type.clone()),
                                    });
                                } else {
                    ...
                    syn::Meta::Path(syn::Path { segments, .. }) => {
                        if let Some(seg) = segments.last() {
                            ...
                            if field_ty == "id" {
                                ...
                                let inner_type = get_type(&ty, true)?;
                                return Ok(Field {
                                    item,
                                    ty: FieldType::ToMany(inner_type.clone()),
                                });
                            } else if field_ty == "to_one" {
                                let inner_type = get_type(&ty, false)?;
                                return Ok(Field {
                                    item,
                                    ty: FieldType::ToOne(inner_type.clone()),
                                });
...
fn get_type(ty: &syn::Type, is_to_many: bool) -> syn::Result<&syn::Ident> {
    ...
    let (outer_ty, inner_ty) = if let syn::Type::Path(syn::TypePath {
    ...
    let (invalid_vec, error_info): (&Vec<&str>, EntityDecoratorError) = if is_to_many {
        (
            &VALID_TO_MANY_WRAPPER,
            EntityDecoratorError::InvalidToManyType,
        )
    } else {
        (
            &VALID_TO_ONE_WRAPPER,
            EntityDecoratorError::InvalidToOneType,
        )
    };
